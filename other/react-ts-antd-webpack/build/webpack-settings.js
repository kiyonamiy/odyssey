...
const argv = require('yargs-parser')(process.argv.slice(4))     // [..., '--mode', 'development', '--open'] => { mode: 'development', open: true }
const APP_ENV = argv.env || 'dev'

const STATIC_DOMAIN = APP_ENV === 'prod' ? '.' : ''

module.exports = {
  APP_ENV,
  // index: path.resolve(__dirname, `./../dist/${constants.APP_ENV}/index.html`),
  // assetsRoot: path.resolve(__dirname, `./../dist/${constants.APP_ENV}`),
  assetsPublicPath: APP_ENV === 'dev' ? '/' : `${STATIC_DOMAIN}/`,
  assetsSubDirectory: '',
