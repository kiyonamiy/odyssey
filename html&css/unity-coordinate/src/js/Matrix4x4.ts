...
class Matrix4x4 {
    ...
    public mul(param: Matrix4x4 | Vector4): Matrix4x4 | Vector4 {
        // 右乘矩阵
        if(param instanceof Matrix4x4) {
            const newMatrix: Matrix4x4 = new Matrix4x4();
            for(let row: number = 0; row < Matrix4x4.DIMENSION; row ++) {
                for(let col: number = 0; col < Matrix4x4.DIMENSION; col ++) {
                    for(let k: number = 0; k < Matrix4x4.DIMENSION; k ++) {
